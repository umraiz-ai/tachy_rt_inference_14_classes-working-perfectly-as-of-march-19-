# PPE safety classification from NPU object detections
import cv2
...
def resolve_ppe_class_ids(clss_dict: dict) -> dict:
    """Map PPE role names to class IDs by looking up class dict values."""
    name_to_id = {v.lower(): int(k) for k, v in clss_dict.items()}
    class_ids = {}
    for role, aliases in PPE_ROLE_NAMES.items():
        found = None
        ...
        if found is None:
            raise KeyError(
                f"Could not resolve PPE role '{role}' from class dict. "
...
def detect_ppe(
    ...
    """
    PPE safety check from pre-computed NPU detections:
      All eligible workers must wear helmets -> missing_helmet
      No eligible workers -> safe
    """
    image = image.copy()
    h, w = image.shape[:2]
