...
class Decoder:
    ...
    def split_logits(self, x, n, n_channels=(4, 80)):  # 4, 4 -> 4, 80
        # Calculate actual channels based on input size
        total_elements = len(x)
        box_elements = n * n_channels[0]
        cls_elements = total_elements - box_elements
        actual_cls_channels = cls_elements // n if cls_elements > 0 else n_channels[1]
        
        return np.reshape(x[:box_elements], (-1, n_channels[0])), np.reshape(x[box_elements:], (-1, actual_cls_channels))
